...
W90LIB = '/home/birolt/li000269/work/script/pyWannier90/src'
...
class W90:
    ...
    def get_hamiltonian_kpts(self):
        ...
        for k_id in range(self.num_kpts_loc):
            ...
            U_matrix_opt = self.U_matrix_opt[k_id][:, :np.sum(orbs_in_win)].T
        ...
        #hamiltonian_kpts = np.einsum('kso,ko,kot->kst', self.U_matrix.conj(), eigenvals_in_window, self.U_matrix)  
    ...
    def get_wannier(self, spinor_mode='total', spinor_phase=True, supercell=[1,1,1], grid=None):
        ...
        if self.spinors:
            ...
            for k_id in range(self.num_kpts_loc):
                ...
                U_matrix_opt = self.U_matrix_opt[k_id][:, :np.sum(mo_in_window)].T
    ...
    def get_wannier_mod(self, spinor_mode='total', spinor_phase=True, supercell=[1,1,1], grid=None):
        '''
        Evaluate the MLWF using a periodic grid
        Args:
            spinor_mode        : 'up' or 'down' or 'total', only applicable for the spinors wave function
        ...
        if self.spinors:
            ...
            if spinor_mode == 'up':
                ...
                WF0 = np.sqrt(WF0_nc_up_sq + WF0_nc_down_sq) 
        else:
            u_mo  = []   
            for k_id in range(self.num_kpts_loc):
                mo_in_window = self.lwindow[k_id]
